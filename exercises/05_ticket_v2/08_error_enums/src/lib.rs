...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    ...
    match ticket {
        Ok(ticket) => { ticket }
        Err(TitleError { message }) => {
            panic!("{message}");
        }
        Err(DescriptionError { message }) => {
...
impl Ticket {
    pub fn new(
        ...
        if title.len() > 50 {
            return Err(TitleError { message: "Title cannot be longer than 50 bytes".to_string() });
        }
        if description.is_empty() {
            return Err(DescriptionError { message: "Description cannot be empty".to_string() });
        }
        if description.len() > 500 {
            return Err(DescriptionError { message: "Description cannot be longer than 500 bytes".to_string() });
        }
